...
/*
 * API for retrieving all articles in {category} collection of the database
 *
 * HTTP Request: GET /articles/{category}
 * HTTP Response:
 ...
 */
@RestController
public class ReadQuestionInCategoryController {

	@Autowired
	private QuestionRepositoryService repository;

	@RequestMapping(value = "/articles/{category}", method = RequestMethod.GET)
	public List<Question> readArticleInCategory(@PathVariable String category) throws ResourceNotExistException, NoContentException{
	    ...
	    // retrieve all articles under {category}
	    // when no articles found, NoContentException is thrown
        QuestionSpecification readAllArticleSpec = QuestionSpecificationFactory.getFindQuestionByCategorySpecification(article);
